		...
		<view class="left">
			<scroll-view class="leftScroll" style="height: 100vh;" :scroll-top="scrollTop" scroll-y="true">
				<view :class="{type1:true, current:(index1 === i)}" :style="{lineHeight: type1LineHeight, fontSize: type1Size, height: type1Height, color: selected(i)}"
				 v-for="(item,i) in list" :key="i" @click="changeType1(item,i)">{{item.name}}</view>
			</scroll-view>
		</view>
		<view class="right">
			<scroll-view class="rightScroll" style="height: 100vh;" :scroll-top="scrollTop" scroll-y="true">
				<view class="type2" v-for="(item,i) in data2" :key="i">
					...
					<view class="type3Box">
						<view class="type3" v-for="(item1,i1) in item.childlist" :key="i1" @click="selectType1(i,i1)">
		...
		methods: {
			changeType1(item, index) {
				...
				if (!item.childlist) {
					...
					// console.log(1111,item.childlist[0].childlist[item.childlist[0].length-1])
					// 修改图片地址
					let url = item.childlist[0]
					let k = url.childlist[url.childlist.length - 1].image.split(',')[0]
					item.childlist[0].childlist[url.childlist.length - 1].image = k
					item.childlist[0].childlist[url.childlist.length - 1].name = '卫衣'
					this.data2 = item.childlist
				}
			},
			selectType1(i, i1) {
